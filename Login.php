    ...
    elseif (strlen($pass) < 5 || strlen($pass) > 8)
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            if(isset($_POST['chkRememberMe'])){
                setcookie('ckEmail',$email,time()+ 60*60); //1 hour expiry
                setcookie('ckPass',$pass,time()+ 60*60); //1 hour expiry
            }
            if ($row['userType'] == 'A')
                header('location: ./Admin/Dashboard.php');
            elseif ($row['userType'] == 'U')
...
<body>
    <h1 align="center">ABC Insurance Login Form</h1>
    <hr>
    <form method="POST">
        <!-- Email input -->
        <div class="form-outline mb-4">
            <input type="text" name="txtEmail" class="form-control" required="" value="<?php if(isset($_COOKIE['ckEmail'])) echo $_COOKIE['ckEmail']; ?>"/>
            <label class="form-label" for="form2Example1">Email Address</label>
        </div>

        <!-- Password input -->
        <div class="form-outline mb-4">
            <input type="password" name="txtPass" class="form-control" required="" value="<?php if(isset($_COOKIE['ckPass'])) echo $_COOKIE['ckPass']; ?>"/>
